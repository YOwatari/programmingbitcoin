...
import (
	...
	"encoding/hex"
	"fmt"
	"github.com/YOwatari/programmingbitcoin/tx"
)
...
func ExampleChapter5_five() {
	const serializedTx string = "010000000456919960ac691763688d3d3bcea9ad6ecaf875df5339e" +
	...
	r := bytes.NewReader(deserialize(serializedTx))
	t, err := tx.ParseTransaction(r, true)
	if err != nil {
		panic(err)
	}

	fmt.Printf("input script 1: %s\n", t.Inputs[1].ScriptSig)
	fmt.Printf("output pubkey 0: %s\n", t.Outputs[0].ScriptPubKey)
	fmt.Printf("output amount 1: %d\n", t.Outputs[1].Amount)

	// Output:
	// input script 1: 304402207899531a52d59a6de200179928ca900254a36b8dff8bb75f5f5d71b1cdc26125022008b422690b8461cb52c3cc30330b23d574351872b7c361e9aae3649071c1a71601 035d5c93d9ac96881f19ba1f686f15f009ded7c62efe85a872e6a19b43c15a2937
	// output pubkey 0: OP_DUP OP_HASH160 ab0c0b2e98b1ab6dbf67d4750b0a56244948a879 OP_EQUALVERIFY OP_CHECKSIG
	// output amount 1: 40000000
}
